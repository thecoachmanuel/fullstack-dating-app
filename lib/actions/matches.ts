...
export async function likeUser(toUserId: string) {
  ...
  if (likeError) {
    throw new Error(`Failed to create like: ${likeError.message || "unknown error"}`);
  }
  ...
  if (existingLike) {
    ...
    return {
      ...
      duplicate: (likeRows?.length ?? 0) === 0,
    };
  }

  return { success: true, isMatch: false, duplicate: (likeRows?.length ?? 0) === 0 };
}
